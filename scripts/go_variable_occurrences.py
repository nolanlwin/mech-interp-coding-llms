"""
Per-site boolean-flag variable occurrences for Go (tree-sitter).
"""
...
from go_boolean_flag_roles import (
    GoFlagHit,
    collect_flag_hits,
    collect_return_hits,
)
...
def _is_indexing_name(node: Node, parents: dict[Node, Node | None]) -> bool:
    p = parents.get(node)
    return p is not None and p.type in {"index_expression", "selector_expression"}
...
def iter_hit_names(h: GoFlagHit) -> list[Node]:
...
def occurrence_rows_from_go_code(
    code: str,
    *,
    repo: str | None = None,
    path: str | None = None,
